...
Tiered 模式和 Leveled 模式在上一节 [3.2 Compaction 策略介绍](3.comlaction.md) 中已经进行过介绍，Tiered 类型更适合数据量较少的情况，当 Mini SSTable 的数据量较大，并与 Minor SSTable 的数据量相近时，则选择 Leveled 类型，减少读放大问题。可以根据实际情况判断选择哪种类型的 Minor Compaction。
